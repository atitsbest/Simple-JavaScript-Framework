// ==============================================
// Erweiterungen zum Number-Object

// Trennzeichen
Number.groupSeparator = '.';
Number.decimalSeparator = ',';

// Bevor wir Methoden überschreiben, merken wir uns die alten.
_oldParseFloat = parseFloat;
_oldParseInt = parseInt;

// ----------------------------------------------
// parseInt
parseFloat = function(str, radix) {
  if (typeof str == 'string') {
    // Trennzeichen austauschen
    str = str
      .replace(eval('/\\' + Number.groupSeparator + '/'), '')
      .replace(eval('/\\' + Number.decimalSeparator + '/'), '.');
  }
  return _oldParseFloat(str, radix);
};
...
Number.prototype.format = function(format) {
	...
	return fnum.toString()
	  .replace(/\./, '@')
	  .replace(/,/, Number.groupSeparator)
	  .replace(/@/, Number.decimalSeparator);
};